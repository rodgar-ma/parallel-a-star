...
path *astar_search(AStarSource *source, int start_id, int goal_id, double *cpu_time_used) {
    ...
    while(!heap_is_empty(open)) {
        ...
        printf("\n%d: Nodo actual: (%d, %d), fCost = %f\n", steps, current->id % WIDTH, current->id / WIDTH, current->fCost);
